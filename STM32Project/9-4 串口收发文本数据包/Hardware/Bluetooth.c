...
//uint8_t Bluetooth_TxPacket[4];
char Bluetooth_RxPacket[100];		//接受缓存区
...
//	Bluetooth_SendArray(Bluetooth_TxPacket, 4);
//	Bluetooth_SendByte(0xFE);
//}
//	
...
void USART1_IRQHandler(void)
{
	...
	{
		uint8_t RxData = USART_ReceiveData(USART1);		//接受串口USART1的数据内容
		...
		if (RxState == 0)
		{
			...
			{
				RxState = 1;
				pRxPacket = 0;		//如果收到，状态改为0，否则维持状态0
			}
		...
		else if (RxState == 1)
		{
			...
			//由于是文本数据接受，不确定是否有包尾，因此 通过是否包尾检测判断
			if (RxData == '\r')
			{
				RxState = 2;
